...
public class PhotosThumbAdapter extends BaseRecyclerAdapter<PhotosThumbAdapter.PhotosThumbViewHolder> {
    ...
    public BaseRecyclerViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new PhotosThumbViewHolder(View.inflate(context, R.layout.vim_photos_grid_item, null));
    }
    ...
    public void onBindViewHolder(BaseRecyclerViewHolder holder, int position) {
        ...
        if (position == 0){
            viewHolder.img.setImageResource(R.mipmap.vim_icon_takephotos);
            viewHolder.checkBox.setVisibility(View.GONE);
        }else {
            ImageUtil.loadViewLocalIfExists(context, curDir + File.separator + photos.get(position), viewHolder.img, R.mipmap.vim_pictures_default);
            if (isMulti) {

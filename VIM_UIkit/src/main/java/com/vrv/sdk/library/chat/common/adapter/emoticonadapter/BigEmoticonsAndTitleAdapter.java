...
public class BigEmoticonsAndTitleAdapter extends EmoticonsAdapter<EmoticonEntity> {
    ...
    protected void bindView(int position, ViewHolder viewHolder) {
        ...
        final EmoticonEntity emoji = mData.get(position);
        if (isDelBtn) {
            viewHolder.iv_emoticon.setImageResource(R.mipmap.icon_del);
            viewHolder.iv_emoticon.setBackgroundResource(com.keyboard.view.R.drawable.bg_emoticon);
        } else {
            if (emoji != null) {
                Glide.with(viewHolder.iv_emoticon.getContext()).load((int) emoji.getEventType()).into(viewHolder.iv_emoticon);
                viewHolder.tv_content.setVisibility(View.VISIBLE);
                viewHolder.tv_content.setText(emoji.getContent());
                viewHolder.iv_emoticon.setBackgroundResource(com.keyboard.view.R.drawable.bg_emoticon);
            }
        }
        ...
        viewHolder.rootView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (mOnEmoticonClickListener != null) {
                    mOnEmoticonClickListener.onEmoticonClick(emoji, ExpressionType.EMOTICON_CLICK_TASK, isDelBtn);
                }
            }
        });
    }
